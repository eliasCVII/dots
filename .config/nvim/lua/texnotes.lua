...
function EditNote()
  tex = GetPath("tex")
  vim.ui.select(tex, { prompt = "choose note to edit" },
    function(choice)
      if choice ~= nil then
...
function RenderNote() -- render a specific note
  tex = GetPath("tex")
  vim.ui.select(tex, { prompt = "choose note to render" },
    function(choice)
      if choice ~= nil then
...
function DeleteFile()
  ...
  tex = GetPath("tex")
  vim.ui.select(tex, { prompt = "choose note to delete" },
    function(choice)
      if choice ~= nil then
        local name = RemovePath(choice):sub(1,-5)
        local command = "!bash auto_delete.sh "..name
        vim.ui.input(
          { prompt = "Deleting "..name..", you sure? (y/n) ",
          },
          function(input)
            if input == "y" then
              vim.cmd(command)
...
function ViewNote()
  pdfs = GetPath("pdf")
  vim.ui.select(pdfs, { prompt = "choose compiled note to view" },
    function(choice)
      if choice ~= nil then
        local name = RemovePath(choice)
        print("opening", name)
...
function RenameNote()
  tex = GetPath("tex")
  vim.ui.select(tex, { prompt = "choose a note to rename" },
    function(choice)
      if choice ~= nil then
...
function RenameReference()
  tex = GetPath("tex")
  vim.ui.select(tex, { prompt = "choose a reference to rename" },
    function(choice)
      if choice ~= nil then
...
-- Menu for Notes
api.nvim_create_user_command('Note',
  function(opts)
    selection = opts.fargs[1]

    -- Set a filewatcher on a .tex file
    if selection == "render" then
      RenderNote()
    ...
    -- Rename a reference
    elseif selection == 'rename_ref' then
      RenameReference()
  ...
  { nargs = 1,
    complete = function(ArgLead, CmdLine, CursorPos)
      ...
      return { "render", "new", "rename", "rename_ref", "delete", "view", "graph", "edit" }
    end,
})
